var Xiaobang = {
    ...
    create_Send: function(data) {
        ...
        if (data.content.text) {
            cont = '<div class="textbox">' + data.content.text + '</div>'
        } else if (data.content.imageUrl) {
            if (data.content.isprepare) {
                cont = '<div class="textbox"><img style="max-width:100%;display:block" src="' + data.content.imageUrl + '" />' +
                    '<div class="imgsendmask"><span></span></div></div>';
            } else {
                cont = '<div class="textbox"><img style="max-width:100%;display:block" src="' + data.content.imageUrl + '" /></div>';
            }
        } else if (data.content.voicePath) {
            cont = '<div class="textbox voicebox" data-path='+data.content.voicePath+' style="width:20%">'+data.content.duration+'S</div>';
        }
        var html = '<div class="send">' +
            '<img src="' + pic + '" class="head" alt="">' + cont + '</div>';
        return html;
    },
